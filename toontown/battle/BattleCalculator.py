...
class BattleCalculator:
    ...
    def __calcToonAtkHit(self, attackIndex, atkTargets):
        ...
        if atkTrack != LURE and atkTrack != HEAL:
            if atkTrack != DROP:
                ...
                attack[TOON_ACCBONUS_COL] = 1
                return (0, 0)
        # if propAcc > MaxToonAcc:
            # propAcc = MaxToonAcc
        if randChoice < propAcc:
            if debug:
                self.notify.debug('HIT: Toon attack rolled ' + str(randChoice) + ' to hit with an accuracy of ' + str(propAcc))
            attack[TOON_ACCBONUS_COL] = 0
        else:
            if debug:
                self.notify.debug('MISS: Toon attack rolled ' + str(randChoice) + ' to hit with an accuracy of ' + str(propAcc))
            attack[TOON_ACCBONUS_COL] = 1
        return (not attack[TOON_ACCBONUS_COL], propAcc)
            ...
            if atkTrack == LURE:
                ...
                if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[TOON_TRACK_COL] == PETSOS:
                    ...
                    if suit:
                        ...
                        if numLeft < 0:
                            numLeft = 0
                        toon.setPinkSlips(numLeft)
                        if costToFire > abilityToFire:
                            self.notify.warning('Not enough PinkSlips to fire cog')
                        else:
                            suit.skeleRevives = 0
    ...
    def __getToonTargets(self, attack):
        ...
        if track == HEAL or track == PETSOS:
            return self.battle.activeToonIds
        else:
    ...
    def __clearAttack(self, attackIdx, toon = 1):
        if toon:
            ...
            longest = max(len(self.battle.activeToonIds), len(self.battle.activeSuits))
    ...
    def __calcSuitTarget(self, attackIndex):
        ...
        if suitId in self.SuitAttackers and random.randint(0, 99) < 75:
            ...
            return self.battle.activeToonIds.index(toonId)
        else:
    ...
    def __pickRandomToon(self, suitId):
        ...
        for currToon in self.battle.activeToonIds:
            if not self.__combatantDead(currToon, toon=1):
                liveToons.append(self.battle.activeToonIds.index(currToon))

        if len(liveToons) == 0:
            self.notify.debug('No tgts avail. for suit ' + str(suitId))
            return -1
        chosen = random.choice(liveToons)
        self.notify.debug('Suit randomly attacking toon ' + str(self.battle.activeToonIds[chosen]))
        return chosen
    ...
    def __createSuitTargetList(self, attackIndex):
        ...
        if not self.__suitAtkAffectsGroup(attack):
            targetList.append(self.battle.activeToonIds[attack[SUIT_TGT_COL]])
            if debug:
    ...
    def __applySuitAttackDamages(self, attackIndex):
        ...
        if self.APPLY_HEALTH_ADJUSTMENTS:
            for t in self.battle.activeToonIds:
                position = self.battle.activeToonIds.index(t)
    ...
    def __printSuitAtkStats(self):
        ...
        for currTgt in self.suitAtkStats.keys():
            if currTgt not in self.battle.activeToonIds:
                continue
            tgtPos = self.battle.activeToonIds.index(currTgt)
            ...
            if i < len(self.battle.activeSuits):
                ...
                if attack[SUIT_ATK_COL] != NO_ATTACK:
                    if self.__suitAtkAffectsGroup(attack):
                        for currTgt in self.battle.activeToonIds:
                            self.__updateSuitAtkStat(currTgt)

                    else:
                        tgtId = self.battle.activeToonIds[attack[SUIT_TGT_COL]]
    ...
    def __initRound(self):
        ...
        attacks = findToonAttack(self.battle.activeToonIds, self.battle.toonAttacks, FIRE)
        for atk in attacks:
            self.toonAtkOrder.append(atk[TOON_ID_COL])

        for track in range(HEAL, DROP + 1):
            attacks = findToonAttack(self.battle.activeToonIds, self.battle.toonAttacks, track)
            if track == TRAP:
                sortedTraps = []
                for atk in attacks:
        ...
        specials = findToonAttack(self.battle.activeToonIds, self.battle.toonAttacks, NPCSOS)
        toonsHit = 0
        cogsMiss = 0
        for special in specials:
            npc_track = NPCToons.getNPCTrack(special[TOON_TGT_COL])
            if npc_track == NPC_TOONS_HIT:
                BattleCalculator.toonsAlwaysHit = 1
                toonsHit = 1
            elif npc_track == NPC_COGS_MISS:
                BattleCalculator.suitsAlwaysMiss = 1
                cogsMiss = 1

        if self.notify.getDebug():
            self.notify.debug('Toon attack order: ' + str(self.toonAtkOrder))
            self.notify.debug('Active toons: ' + str(self.battle.activeToonIds))
    ...
    def calculateRound(self):
        ...
        if toonsHit == 1:
            BattleCalculator.toonsAlwaysHit = 0
        if cogsMiss == 1:
            BattleCalculator.suitsAlwaysMiss = 0
        if self.notify.getDebug():
    ...
    def __updateActiveToons(self):
        ...
        if not self.CLEAR_SUIT_ATTACKERS:
            ...
            for s in self.SuitAttackers.keys():
                for t in self.SuitAttackers[s].keys():
                    if t not in self.battle.activeToonIds:
        ...
        for trap in self.traps.keys():
            if self.traps[trap][1] not in self.battle.activeToonIds:
