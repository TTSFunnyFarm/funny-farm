...
class Purchase(PurchaseBase):
    ...
    def _moneyAddTask(self, task):
        task.count += 1
        max = self.counters[0].max
        if task.count > max:
            return Task.done
        toon = base.localAvatar
        if toon.getMoney() < toon.getMaxMoney():
            toon.setMoney(toon.getMoney() + 1)
        else:
            toon.setBankMoney(toon.getBankMoney() + 1)
        return Task.cont
    ...
    def countDown(self):
        ...
        moneyAddTask = taskMgr.add(self._moneyAddTask, 'moneyAdd')
        moneyAddTask.count = 0
