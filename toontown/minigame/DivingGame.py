...
class DivingGame(Minigame):
    ...
    def __init__(self):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM('DivingGame', [State.State('off', self.enterOff, self.exitOff, ['swim']), State.State('swim', self.enterSwim, self.exitSwim, ['cleanup']), State.State('cleanup', self.enterCleanup, self.exitCleanup, [])], 'off', 'cleanup')
        ...
        self.dead = 0
        self.score = 0
        self.jellybeanValues = {
            ToontownGlobals.ToontownCentral: 12,
            ToontownGlobals.DaisyGardens: 16,
            ToontownGlobals.TheBrrrgh: 20,
            ToontownGlobals.DonaldsDreamland: 24,
        }
    ...
    def fishCollision(self, collEntry):
        ...
        if name.startswith('crabby'):
            self.handleCrabCollision(toonSD.status)
        else:
    ...
    def setGameReady(self):
        ...
        self.cTrav = CollisionTraverser('DiverGame')
        self.cTrav2 = CollisionTraverser('DiverGame')
    ...
    def timerExpired(self):
        ...
        self.dead = 1
        self.numJellybeans = self.score * self.jellybeanValues[self.getSafezoneId()]
        self.gameOver()
    ...
    def __boatReached(self, collEntry):
        toonSD = self.toonSDs[self.localAvId]
        if toonSD.status == 'treasure':
            self.treasureRecovered()
    ...
    def setTreasureGrabbed(self, avId, chestId):
        ...
        self.treasures[chestId].treasureNode.reparentTo(self.getAvatar())
        headparts = self.getAvatar().getHeadParts()
        pos = headparts[2].getPos()
    ...
    def __crabTask(self, task):
        ...
        for crab in self.crabs:
            if not crab.moveLerp.isPlaying():
                ...
                crab.moveLerp.loop()
                self.getCrabMoving(crab.crabId, crab.getX(), crab.direction)
                return Task.cont
    ...
    def getCrabMoving(self, crabId, crabX, dir):
        ...
        self.setCrabMoving(crabId, timestamp, rand1, rand2, crabX, dir)
    ...
    def treasureRecovered(self):
        ...
        self.score += 1
        self.incrementScore(avId, newSpot, timestamp)

    def pickupTreasure(self, chestId):
        avId = self.localAvId
        self.setTreasureGrabbed(avId, chestId)
    ...
    def spawnFish(self, spawnerId):
        ...
        for i in xrange(len(props)):
            ...
            if num > low and num <= high:
                ...
                self.fishSpawn(timestamp, i, spawnerId, offset)
                return
    ...
    def handleCrabCollision(self, status):
        ...
        if status == 'normal' or status == 'treasure':
            timestamp = globalClockDelta.getFrameNetworkTime()
            self.performCrabCollision(avId, timestamp)
        self.dropTreasure()

    def handleFishCollision(self, spawnId, spawnerId, status):
        avId = self.localAvId
        timestamp = globalClockDelta.getFrameNetworkTime()
        self.performFishCollision(avId, spawnId, spawnerId, timestamp)
        self.dropTreasure()
    ...
    def dropTreasure(self):
        ...
        timestamp = globalClockDelta.getFrameNetworkTime()
        if self.hasTreasure:
            self.setTreasureDropped(avId, timestamp)
