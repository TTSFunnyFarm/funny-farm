...
class MazeGame(Minigame):
    ...
    def __init__(self):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM('MazeGame', [State.State('off', self.enterOff, self.exitOff, ['play']),
    ...
    def load(self):
        ...
        mazeIndex = mazeMap[self.getSafezoneId()]
        mazeName = MazeGameGlobals.getMazeName(self.doId, mazeIndex, MazeData.mazeNames)
        self.maze = Maze.Maze(mazeName)
        model = loader.loadModel(MazeGameGlobals.IconMap[self.getSafezoneId()][0])
        self.treasureModel = model.find('**/%s' % MazeGameGlobals.IconMap[self.getSafezoneId()][1])
        model.removeNode()
    ...
    def enterPlay(self):
        ...
        self.orthoWalk = OrthoWalk(orthoDrive, broadcast=0)
    ...
    def __treasureGrabbed(self, treasureNum):
        ...
        self.claimTreasure(treasureNum)
    ...
    def __showToonHitBySuit(self, avId, timestamp):
        ...
        if avId != self.localAvId:
            ...
            def camTask(task, zenith = zenith, flyNode = toon, startCamPos = startCamPos, camOffset = destCamPos - startCamPos):
                ...
                camera.lookAt(toon)
                return Task.cont
            camTaskName = 'mazeToonFlyCam-' + str(avId)
        ...
        def postFunc(self = self, avId = avId, oldGeomNodeZ = oldGeomNodeZ, dropShadow = dropShadow, parentNode = parentNode):
            if avId == self.localAvId:
                ...
                base.localAvatar.setAnimState('Happy', 1.0)
    ...
    def enterShowScores(self):
        ...
        self.numJellybeans = self.scores[0] * self.jellybeanValues[self.getSafezoneId()]
        lerpTrack = Parallel()
        lerpDur = 0.5
