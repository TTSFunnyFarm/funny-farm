...
class TugOfWarGame(Minigame):
    ...
    def __init__(self):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM('TugOfWarGame', [State.State('off', self.enterOff, self.exitOff, ['waitForGoSignal']),
    ...
    def unload(self):
        ...
        aspect2d.find('**/MinigamePowerMeter*').removeNode() # Since the power meter doesn't want to fucking cooperate
        del self.powerMeter
    ...
    def setGameReady(self):
        ...
        if self.isSinglePlayer():
            ...
            if randInt < 8:
                ...
                self.gameType = TugOfWarGameGlobals.TOON_VS_TOON
                self.kMovement = 0.04
        self.sendGameType(self.gameType, self.suitType)
    ...
    def enterTug(self):
        ...
        taskMgr.doMethodLater(TugOfWarGameGlobals.TUG_TIMEOUT, self.tugTimeoutTask, self.uniqueName('tug-timeout'))
        if self.suit:
    ...
    def exitTug(self):
        ...
        self.timer.stop()
        self.timer.hide()
        taskMgr.remove(self.uniqueName('tug-timeout'))
    ...
    def __updateTimerTask(self, task):
        ...
        self.reportCurrentKeyRate(self.keyRate, self.currentForce)
        self.setSpeedGauge()
    ...
    def __spawnUpdateTimerTask(self):
        taskMgr.remove(self.uniqueName(self.UPDATE_TIMER_TASK))
        taskMgr.doMethodLater(TugOfWarGameGlobals.SEND_UPDATE, self.__updateTimerTask, self.uniqueName(self.UPDATE_TIMER_TASK))

    def __killUpdateTimerTask(self):
        taskMgr.remove(self.uniqueName(self.UPDATE_TIMER_TASK))

    def __spawnUpdateKeyPressRateTask(self):
        taskMgr.remove(self.uniqueName(self.UPDATE_KEY_PRESS_RATE_TASK))
        taskMgr.doMethodLater(0.1, self.__updateKeyPressRateTask, self.uniqueName(self.UPDATE_KEY_PRESS_RATE_TASK))
    ...
    def __spawnUpdateIdealRateTask(self):
        ...
        taskMgr.doMethodLater(self.targetRateList[self.nextRateIndex][0], self.__updateIdealRateTask, self.uniqueName('targetRateTimer'))
    ...
    def __killUpdateIdealRateTask(self):
        taskMgr.remove(self.uniqueName('targetRateTimer'))
        ...
        if avId == None:
            if self.suitId not in self.fallenList:
                ...
                if curPos[0] < 0 and curPos[0] > -2 or curPos[0] > 0 and curPos[0] < 2:
                    ...
                    self.throwInWater()
                    losingSide = 1
                    self.reportEndOfContest(losingSide)
    ...
    def __spawnUpdateRopeTask(self):
        taskMgr.remove(self.uniqueName(self.UPDATE_ROPE_TASK))
        taskMgr.add(self.__updateRopeTask, self.uniqueName(self.UPDATE_ROPE_TASK))
    ...
    def __spawnMouseSpeedTask(self):
        taskMgr.remove(self.uniqueName('mouseSpeed'))
        taskMgr.add(self.__mouseSpeedTask, self.uniqueName('mouseSpeed'))
    ...
    def reportCurrentKeyRate(self, keyRate, force):
        ...
        self.forceDict[self.sides[avId]][avId] = force
        self.remoteKeyRateUpdate(avId, self.keyRateDict[avId])
        self.howManyReported += 1
        if self.howManyReported == self.numPlayers:
            self.howManyReported = 0
            self.calculateOffsets()
            self.sendCurrentPosition(self.offsetDict.keys(), self.offsetDict.values())
            if self.gameType == TugOfWarGameGlobals.TOON_VS_COG:
                self.sendSuitPosition(self.suitOffset)
    ...
    def enterContestOver(self):
        ...
        self.sendStopSignal(self.winners, self.losers, self.tieers)
