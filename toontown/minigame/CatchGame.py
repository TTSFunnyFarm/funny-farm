...
class CatchGame(Minigame):
    ...
    def __init__(self):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM('CatchGame', [State.State('off', self.enterOff, self.exitOff, ['play']), State.State('play', self.enterPlay, self.exitPlay, ['cleanup']), State.State('cleanup', self.enterCleanup, self.exitCleanup, [])], 'off', 'cleanup')
    ...
    def showCatch(self, avId, objNum):
        ...
        if objType.good:
            if objNum not in self.droppedObjCaught:
                ...
                self.setObjectCaught(avId, objNum)
        else:
    ...
    def setEveryoneDone(self):
        ...
        def endGame(task, self = self):
            if not CatchGameGlobals.EndlessGame:
                self.numJellybeans = self.scores[0]
                self.gameOver()
            return Task.done
