...
class CannonGame(Minigame):
    ...
    def __init__(self):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM('CannonGame', [State.State('off', self.enterOff, self.exitOff, ['aim']),
    ...
    def __createToonModels(self, avId):
        ...
        head.setupHead(self.getAvatar().style)
        ...
        self.getAvatar().dropShadow.hide()
    ...
    def enterShoot(self):
        ...
        avId = self.localAvId
        self.setCannonWillFire(avId, self.getCurrentGameTime(), self.cannonDict[avId][0].getH(), self.cannonDict[avId][1].getP())
    ...
    def __updateRewardCountdown(self, task):
        ...
        if task.curScore != score:
            ...
            self.jarIval.start()
        task.curScore = score
        self.numJellybeans = task.curScore
        return Task.cont
