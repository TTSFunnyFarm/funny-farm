...
class ToonData(yaml.YAMLObject):
    ...
    def encrypt(self):
        self.setDNA = binascii.hexlify(str(self.setDNA).encode())
        self.setName = binascii.hexlify(self.setName.encode())
        ...
        self.setInventory = binascii.hexlify(str(self.setInventory).encode())
        self.setExperience = binascii.hexlify(str(self.setExperience).encode())
        self.setTrackAccess = binascii.hexlify(str(self.setTrackAccess).encode())
        self.setNametagStyle = binascii.hexlify(self.setNametagStyle.encode())
        self.setCheesyEffect = bin(self.setCheesyEffect)
        self.setLastHood = bin(self.setLastHood)
        self.setHat = binascii.hexlify(str(self.setHat).encode())
        self.setGlasses = binascii.hexlify(str(self.setGlasses).encode())
        self.setBackpack = binascii.hexlify(str(self.setBackpack).encode())
        self.setShoes = binascii.hexlify(str(self.setShoes).encode())
        self.setLevel = bin(self.setLevel)
        self.setLevelExp = bin(self.setLevelExp)
        self.setDamage = binascii.hexlify(str(self.setDamage).encode())
        self.setDefense = binascii.hexlify(str(self.setDefense).encode())
        self.setAccuracy = binascii.hexlify(str(self.setAccuracy).encode())
        self.setClothesTopsList = binascii.hexlify(str(self.setClothesTopsList).encode())
        self.setClothesBottomsList = binascii.hexlify(str(self.setClothesBottomsList).encode())
        self.setHatList = binascii.hexlify(str(self.setHatList).encode())
        self.setGlassesList = binascii.hexlify(str(self.setGlassesList).encode())
        self.setBackpackList = binascii.hexlify(str(self.setBackpackList).encode())
        self.setShoesList = binascii.hexlify(str(self.setShoesList).encode())
        self.setQuests = binascii.hexlify(str(self.setQuests).encode())
        self.setQuestHistory = binascii.hexlify(str(self.setQuestHistory).encode())
        self.setQuestCarryLimit = bin(self.setQuestCarryLimit)
        self.setQuestingZone = bin(self.setQuestingZone)
        self.setTrackProgress = binascii.hexlify(str(self.setTrackProgress).encode())
        self.setHoodsVisited = binascii.hexlify(str(self.setHoodsVisited).encode())
        self.setTeleportAccess = binascii.hexlify(str(self.setTeleportAccess).encode())
        self.setFishingRod = bin(self.setFishingRod)
        self.setFishCollection = binascii.hexlify(str(self.setFishCollection).encode())
        self.setFishTank = binascii.hexlify(str(self.setFishTank).encode())
        self.setTutorialAck = bin(self.setTutorialAck)

    def decrypt(self):
        self.setDNA = ast.literal_eval(binascii.unhexlify(self.setDNA).decode())
        ...
        self.setTrackAccess = ast.literal_eval(binascii.unhexlify(self.setTrackAccess).decode())
