...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def enterCloseBook(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        bookTracks = Parallel()
        for bookActor in self.getBookActors():
            bookTracks.append(ActorInterval(bookActor, 'book', playRate = 1.1, startTime=4.96))

        bookTracks.append(ActorInterval(self, 'book', playRate = 1.1, startTime=4.96))
        if hasattr(self, 'uniqueName'):
    ...
    def enterSquish(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        if callback:
            self.track.setDoneEvent('squish-done')
            self.acceptOnce('squish-done', callback, extraArgs)
        self.track.start(ts)
        self.setActiveShadow(1)
