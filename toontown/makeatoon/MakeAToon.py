...
class MakeAToon(StateData.StateData):
    ...
    def __init__(self, doneEvent, index, isPaid):
        self.isPaid = isPaid
        ...
        self.ns = NameShop.NameShop('NameShop-done', index)
    ...
    def enter(self):
        ...
        musicMgr.playMusic(self.music, looping=1, volume=self.musicVolume)
    ...
    def __handleNameShopDone(self):
        ...
        if self.ns.getDoneStatus() == 'last':
            self.ns.hideAll()
            self.goToLastShop()
        elif self.ns.getDoneStatus() == 'done-tutorial':
            self.doneStatus = 'created'
            base.transitions.fadeOut()
            Sequence(Wait(1), Func(self.__handleTutorial)).start()
        else:
            self.doneStatus = 'created'
            base.transitions.fadeOut()
            Sequence(Wait(1), Func(self.__handleDone)).start()
