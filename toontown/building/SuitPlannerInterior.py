...
class SuitPlannerInterior:
    ...
    def _genSuitInfos(self, numFloors, bldgLevel, bldgTrack):
        ...
        for currFloor in range(numFloors):
            ...
            if self.dbg_4SuitsPerFloor:
                ...
                numActive = random.randint(1, min(2, len(lvls)))
            if currFloor + 1 == numFloors and len(lvls) > 1:
                ...
                self.notify.debug('Adjusted LevelList: ' + repr(lvls))
    ...
    def __genSuitObject(self, suitType, bldgTrack, suitLevel, revives = 0):
        newSuit = BattleSuit()
    ...
    def genFloorSuits(self, floor):
        ...
        for activeSuitInfo in floorInfo['activeSuits']:
            suit = self.__genSuitObject(activeSuitInfo['type'], activeSuitInfo['track'], activeSuitInfo['level'], activeSuitInfo['revives'])
            activeSuits.append(suit)

        suitHandles['activeSuits'] = activeSuits
        reserveSuits = []
        for reserveSuitInfo in floorInfo['reserveSuits']:
            suit = self.__genSuitObject(reserveSuitInfo['type'], reserveSuitInfo['track'], reserveSuitInfo['level'], reserveSuitInfo['revives'])
