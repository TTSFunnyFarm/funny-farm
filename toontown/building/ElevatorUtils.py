...
def getLeftOpenInterval(leftDoor, type):
    openTime = ElevatorData[type]['openTime']
    closedPos = getLeftClosePoint(type)
    openPos = getLeftOpenPoint(type)
    leftOpenInterval = LerpPosInterval(leftDoor, openTime, openPos, startPos=closedPos, blendType='easeOut', name='leftDoorOpen')
    return leftOpenInterval


def getRightOpenInterval(rightDoor, type):
    openTime = ElevatorData[type]['openTime']
    closedPos = getRightClosePoint(type)
    openPos = getRightOpenPoint(type)
    rightOpenInterval = LerpPosInterval(rightDoor, openTime, openPos, startPos=closedPos, blendType='easeOut', name='rightDoorOpen')
    return rightOpenInterval


def getOpenInterval(leftDoor, rightDoor, openSfx, finalOpenSfx, type = ELEVATOR_NORMAL):
    left = getLeftOpenInterval(leftDoor, type)
    right = getRightOpenInterval(rightDoor, type)
...
def getLeftCloseInterval(leftDoor, type):
    closeTime = ElevatorData[type]['closeTime']
    closedPos = getLeftClosePoint(type)
    openPos = getLeftOpenPoint(type)
    leftCloseInterval = LerpPosInterval(leftDoor, closeTime, closedPos, startPos=openPos, blendType='easeOut', name='leftDoorClose')
    return leftCloseInterval


def getRightCloseInterval(rightDoor, type):
    closeTime = ElevatorData[type]['closeTime']
    closedPos = getRightClosePoint(type)
    openPos = getRightOpenPoint(type)
    rightCloseInterval = LerpPosInterval(rightDoor, closeTime, closedPos, startPos=openPos, blendType='easeOut', name='rightDoorClose')
    return rightCloseInterval


def getCloseInterval(leftDoor, rightDoor, closeSfx, finalCloseSfx, type = ELEVATOR_NORMAL):
    left = getLeftCloseInterval(leftDoor, type)
    right = getRightCloseInterval(rightDoor, type)
